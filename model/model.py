...
class RoadSegmentation():
    def __init__(self, device='cpu'):
        ...
        self.model = dsmpnet(3).to(self.device)
        # self.__initialize_mssdmpa_weights(self.model)
    ...
    def train(self, path_to_train: str, path_to_val: str, num_epoch=100, batch_size=64, acc_step=1, lr=0.001):
        ...
        dataloader_val = data.DataLoader(segment_val, batch_size=8, shuffle=False,
                                          num_workers=self.num_workers, pin_memory=self.pin_memory)
        ...
        log_dir = f'meta_data/HRNet/MSSDMPA_Net_3'
        ...
        for count_epoch in range(num_epoch):
            train_metrics = self.run_epoch(count_epoch, dataloader_train, self.optimizer, acc_step, is_train=True)
            val_metrics = self.run_epoch(count_epoch, dataloader_val, None, acc_step, is_train=False)

            # self.scheduler.step(val_metrics['Val_Loss'])
            current_lr = self.optimizer.param_groups[0]['lr']
            self.writer.add_scalar('Learning Rate', current_lr, count_epoch)

            self.__log_metrics(train_metrics, val_metrics, count_epoch+39)
    ...
    def run_epoch(self, epoch_num, dataloader, optimizer, acc_step, is_train=True):
        if is_train:
            self.model.train()
            mode_str = "Train"
        else:
        ...
        for i, (x_batch, y_batch) in enumerate(progress_bar):
            ...
            if is_train:
                ...
                with torch.set_grad_enabled(True):
                    pred1, pred2, pred3, pred4, pred5 = self.model(x_batch)
                    loss_value = y_bce_loss(pred1, pred2, pred3, pred4, pred5, y_batch)                    
                ...
                if accumulated_steps % acc_step == 0 or i == len(dataloader) - 1:
                    ...
                    #     self.__log_gradients(epoch_num)
            ...
            with torch.no_grad():
                preds_binary = (pred1 > 0.5).float()

                intersection = torch.clamp(preds_binary * y_batch, 0, 1)
                union = torch.clamp(preds_binary + y_batch, 0, 1)

                iou_batch = torch.mean(intersection) / (torch.mean(union) + 1e-5)
                ...
                all_iou_scores.append(iou_batch.cpu().tolist())
                # all_iou_scores.extend(iou_batch)
    ...
    def test(self, path_to_test_data: str, batch_size=1, threshold=0.5, save_dir="test_results"):
